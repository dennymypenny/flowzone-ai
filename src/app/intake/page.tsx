      ...
      <div className="min-h-screen bg-white flex items-center justify-center px-6 py-20">
        <div className="max-w-xl text-center">
          ...
          <p className="text-gray-500 text-lg leading-relaxed mb-10">
            ...
            <span className="text-sky-600 font-semibold">{form.email}</span> within{" "}
            <strong className="text-gray-900">24 hours</strong>.
          </p>

          {/* Upsell card */}
          <div className="bg-gradient-to-br from-sky-600 to-sky-700 rounded-2xl p-8 text-left mb-8 shadow-xl">
            ...
            <ul className="space-y-2 mb-6">
              {[
                ...
                <li key={item} className="flex items-center gap-2 text-sky-100 text-sm">
            ...
            <Link
              href="/scan"
              className="block w-full bg-white text-sky-700 font-black py-4 rounded-xl text-center hover:bg-sky-50 transition-colors text-lg"
            >
              Get the AI Scan for $97 →
            </Link>
            <p className="text-sky-300 text-xs text-center mt-3">30-day money-back guarantee</p>
          </div>

          <p className="text-gray-400 text-sm">
            Prefer to talk through it?{" "}
            <Link href="/book" className="text-sky-600 hover:underline font-semibold">
    ...
    <>
      <section className="bg-white pt-20 pb-10 px-6 border-b border-gray-100">
        <div className="max-w-2xl mx-auto text-center">
          <p className="text-sky-600 font-semibold text-sm uppercase tracking-wider mb-4">Free AI Audit</p>
          ...
          <div className="flex items-center gap-2 mb-10">
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center gap-2 flex-1">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${step >= s ? "bg-sky-600 text-white" : "bg-gray-200 text-gray-400"}`}>{s}</div>
                <div className={`h-1 flex-1 rounded-full ${s < 3 ? (step > s ? "bg-sky-600" : "bg-gray-200") : "hidden"}`} />
              </div>
            ))}
          </div>
              ...
              <>
                ...
                <div>
                  ...
                  <select value={form.monthlyRevenue} onChange={(e) => set("monthlyRevenue", e.target.value)}
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-sky-400 transition-colors bg-white">
                ...
                <button type="button" onClick={() => setStep(2)}
                  ...
                  className="w-full bg-sky-600 text-white font-bold py-4 rounded-xl hover:bg-sky-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                  Continue →
                </button>
              </>
            )}
              ...
              <>
                ...
                <div className="flex gap-3">
                  ...
                  <button type="button" onClick={() => setStep(3)}
                    ...
                    className="flex-[2] bg-sky-600 text-white font-bold py-4 rounded-xl hover:bg-sky-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                    Continue →
                  </button>
                </div>
              </>
